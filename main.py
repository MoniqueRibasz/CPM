...
Df = pd.read_excel("https://raw.githubusercontent.com/MoniqueRibasz/CPM/main/Cronograma.xlsx") # ARQUIVO BASE DE DADOS
...
# Criar DataFrame com ES e EF para cada atividade
gantt_data = pd.DataFrame([
    {
        "Task": k,
        "Start": pd.Timestamp("2025-01-01") + pd.to_timedelta(v["ES"], unit="W"), # Tempo Inicio do GANTT
        "Finish": pd.Timestamp("2025-01-01") + pd.to_timedelta(v["EF"], unit="W"), # Tempo Fim do GANTT
        "Critical": "Sim" if k in paths[index_p] else "Não"
    }
    for k, v in node_data.items()
])
